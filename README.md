...
## TODOLIST
- [ ] 提升向量召回的正确性：找得准
  - [ ] 构建测试数据集v2: 中等任务\[语义召回\]，10条测试数据
  - 索引优化：规范文档、分词器
    - [ ] 规范文档：当前按页读pdf，数据语义也不连续，存在换行问题等
    - [ ] 文本向量trick：用GPT提取文本关键信息或构建问题，将这批问题以注释的形式放在文档内，以匹配用户提问问题，以提高向量搜索的准确
  - RetrievalQA研究
- [ ] 并发和异步
- [ ] 出处
...
2023.12.16: 第一阶段，LangChain基本流程打通，构造简单测试集，验证基于es关键词召回(100%)和向量召回(60%)的效果

- [x] 提升召回的正确性：找得对
  - [x] 构建测试数据集v1: 简单任务\[原文召回\]，10条测试数据
  - [x] 向量索引自动化测试脚本: base recall acc@top3=20%，召回标准query按序出现在原文即可
  - 索引优化：规范文档、分词器
    - [x] 向量模型选型：text2vec-base-chinese -> gte_base_zh(当前C-MTEB top2): recall acc@top3=60%(+40%)，**gte模型效果提升40%**
  - [x] **关键词召回: 原文召回100%**
